...
int main() {
    std::string username, password, user_role_str;
    Role user_role;
    ...
    if (choice == 1) {
        ...
        std::cin >> password;
        std::cout << "Enter Role (admin/developer/user): ";
        std::cin >> user_role_str;

        if (registerUser(username, password, user_role_str)) {
    ...
    user_role_str = authenticateUser(username, password);

    if (user_role_str == "invalid") {
        std::cout << "Authentication failed. Exiting...\n";
        return 1;  // Exit program if login fails
    }

    // Convert user_role_str to Role Enum
    if (user_role_str == "admin") {
        user_role = ADMIN;
    } else if (user_role_str == "developer") {
        user_role = DEVELOPER;
    } else if (user_role_str == "user") {
        user_role = USER;
    } else {
        std::cout << "Invalid role detected. Exiting...\n";
        return 1;
    }

    std::cout << "Welcome, " << username << "! You are logged in as: " << user_role_str << "\n";

    // Fix: Clear input buffer before moving to command loop
    ...
    while (true) {
        ...
        switch (command) {
            ...
            if (!hasAccess(user_role, "CREATE")) {
        std::cout << "Access Denied! You cannot create databases.\n";
        break;
    } {
            ...
            if (!hasAccess(user_role, "DROP")) {
        std::cout << "Access Denied! You cannot drop databases.\n";
        break;
    }
            {
                std::stringstream ss(command_input);
            ...
            if (!hasAccess(user_role, "CREATE")) {
        std::cout << "Access Denied! You cannot create tables.\n";
        break;
    } {
                std::stringstream ss(command_input);
            ...
            if (!hasAccess(user_role, "DROP")) {
        std::cout << "Access Denied! You cannot drop tables.\n";
        break;
    }{
                std::stringstream ss(command_input);
            ...
            if (!hasAccess(user_role, "UPDATE")) {
        std::cout << "Access Denied! You cannot modify table structure.\n";
        break;
    } {
                std::stringstream ss(command_input);
            ...
            case CommandType::INSERT:
             if (!hasAccess(user_role, "CREATE")) {
        std::cout << "Access Denied! You cannot insert data.\n";
        break;
    } {
                if (!current_table) {
                    std::cout << "Error: No table selected. Use 'create table' or 'use table'.\n";
            ...
            if (!hasAccess(user_role, "UPDATE")) {
    std::cout << "Access Denied! You cannot delete records.\n";
    break;
}{
                if (!current_table) {
                    std::cout << "Error: No table selected. Use 'create table' or 'use table'.\n";
            ...
            case CommandType::UPDATE: 
             if (!hasAccess(user_role, "UPDATE")) {
        std::cout << "Access Denied! You cannot update records.\n";
        break;
    }{
                if (!current_table) {
                    std::cout << "Error: No table selected. Use 'create table' or 'use table'.\n";
