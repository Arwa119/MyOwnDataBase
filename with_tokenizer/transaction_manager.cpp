...
TransactionManager::TransactionManager(const std::string& db_name) : in_transaction(false), state(TransactionState::ROLLED_BACK), locked_table(nullptr) {
...
bool TransactionManager::beginTransaction(Table* table) {
    if (in_transaction) {
        std::cout << "Error: Transaction already in progress.\n";
        return false;
    }
    if (!try_lock_table(table)) {
        std::cout << "Error: Table is locked by another transaction.\n";
        return false;
    }
    in_transaction = true;
...
bool TransactionManager::commit() {
    if (!in_transaction || state != TransactionState::ACTIVE) {
    ...
    unlock_table(locked_table);
    locked_table = nullptr;
    log_entries.clear();
    std::cout << "Transaction committed.\n";
    return true;
}

bool TransactionManager::rollback(Table* table) {
    if (!in_transaction || state != TransactionState::ACTIVE) {
...
bool TransactionManager::logInsert(const std::string& table_name, uint32_t row_num, const Row& row) {
    if (!in_transaction) {
...
bool TransactionManager::logDelete(const std::string& table_name, uint32_t row_num, const Row& row) {
    if (!in_transaction) {
        return true; // Non-transactional operation
    }
...
bool TransactionManager::logUpdate(const std::string& table_name, uint32_t row_num, const Row& old_row) {
    if (!in_transaction) {
        return true; // Non-transactional operation
    }
