...
int main() {
    ...
    while (true) {
        ...
        if (input == ".exit") {
            ...
            if (txn_manager.isInTransaction()) {
                txn_manager.rollback(current_table);
            }
            std::cout << "Exiting...\n";
            break;
        }
        ...
        switch (command) {
            ...
            case CommandType::DROP_DB: {
                ...
                // Close transaction log for this database
                txn_manager.setCurrentDatabase("master"); // Switch to master to close db_name's log
            ...
            case CommandType::INSERT: {
                ...
                if (txn_manager.isInTransaction()) {
                    txn_manager.logInsert(current_table_name, row_to_insert_at, row);
                }
            ...
            case CommandType::DELETE: {
                ...
                if (row_num_to_delete != std::numeric_limits<uint32_t>::max()) {
                    ...
                    if (removed_from_btree) {
                        ...
                        deserialize_row(row_slot, row);
                        if (txn_manager.isInTransaction()) {
                            txn_manager.logDelete(current_table_name, row_num_to_delete, row);
                        }
                        row.is_deleted = true;
                        serialize_row(row, row_slot);
            ...
            case CommandType::UPDATE: {
                ...
                if (row_num != std::numeric_limits<uint32_t>::max() && !is_row_freed(current_table, row_num)) {
                    ...
                    if (txn_manager.isInTransaction()) {
                        txn_manager.logUpdate(current_table_name, row_num, old_row);
                    }
            ...
            case CommandType::BEGIN_TRANSACTION: {
                ...
                txn_manager.beginTransaction(current_table);
                break;
            }
